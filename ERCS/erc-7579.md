...
- On installation: Smart accounts MUST initialize module states as specified.
- On uninstallation: Smart accounts SHOULD clearly state whether the module's state is preserved or reset, per module documentation.
...
- Unrecoverable Accounts: Smart accounts must ensure the presence of at least one Validation Module to prevent becoming unrecoverable. The absence of a Validation Module can lead to irrecoverable accounts, posing significant risks.
- Modules SHOULD take state management into consideration during installation or uninstallation to avoid unexpected behaviors and potential security vulnerabilities.
